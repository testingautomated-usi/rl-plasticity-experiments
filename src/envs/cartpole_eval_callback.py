...
class CartPoleEvalCallback(EnvEvalCallback):

    # i.e. performance is not adequate when there is a 20% degradation wrt reward threshold
    def __init__(self, reward_threshold=475.0, unacceptable_pct_degradation=20.0):
        self.reward_threshold = reward_threshold
        self.unacceptable_pct_degradation = unacceptable_pct_degradation
        self.logger = Log("CartPoleEvalCallback")
    ...
    def evaluate_env(self, model, env, n_eval_episodes, sb_version="sb2") -> Tuple[bool, Dict]:
        ...
        percentage_drop = 100.0 - (100.0 * mean_reward) / self.reward_threshold if mean_reward < self.reward_threshold else 0.0
        self.logger.debug(
            "Mean reward: {}, Std reward: {}, Percentage drop: {}".format(mean_reward, std_reward, percentage_drop)
        )
        adequate_performance = mean_reward > (
            self.reward_threshold - (self.reward_threshold * self.unacceptable_pct_degradation / 100.0)
        )
        info = dict()
