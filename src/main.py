...
from stable_baselines.results_plotter import X_TIMESTEPS, X_EPISODES

from log_utils import _ts2xy
from utilities import str2bool, check_probability_range, check_param_names, PREFIX_DIR_MODELS_SAVE, SUPPORTED_ALGOS, \
    SUPPORTED_ENVS
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mode", choices=["train", "test", "eval_model"],
    ...
    if args.mode == "train":
        ...
        for current_iteration in range(args.num_training_runs):
            ...
            if args.execute:
                if not args.continue_learning:
                    runner.execute_train_without_evaluation(current_iteration=current_iteration,
                ...
                if args.save_model:
                    if not args.continue_learning:
                        mean_reward, std_reward = runner.execute_test_without_callback(n_eval_episodes=100,
                                                                                       model_path=model_save_path)
                    else:
                        model_save_path_continue_learning = prefix_dir + '_' + args.continue_learning_suffix \
                                                            + '_it_' + str(current_iteration)
                        mean_reward, std_reward = runner.execute_test_without_callback(
                            n_eval_episodes=100,
                            model_path=model_save_path_continue_learning
                        )
        ...
        if args.save_model:
            logger.debug('Average of the mean rewards: {} +- {}'.format(np.mean(np.array(mean_rewards)),
                                                                        np.std(np.array(mean_rewards))))
            logger.debug('Average of the std rewards: {} +- {}'.format(np.mean(np.array(std_rewards)),
                                                                       np.std(np.array(std_rewards))))
        ...
        for model_folder in model_folders:
            ...
            assert x_rewards[-1] == len(rewards_repeated), \
                'x_rewards last element should be equal to the length of rewards repeated: {}, {}'\
                    .format(x_rewards[-1], len(rewards_repeated))
        ...
        plot_title = args.tb_log_name + '_' + args.algo_name + '_' + str(args.num_training_runs) \
                     + '_smooth_' + str(smoothing_window)
