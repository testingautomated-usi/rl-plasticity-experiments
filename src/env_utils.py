    ...
    if evaluate:
        ...
        if not evaluate_during_learning or continue_learning:
            if not os.path.exists(os.path.join(orig_log_dir, 'vecnormalize.pkl')):
                ...
                logger.debug('{} does not exist. Trying to search it in the original model directory {}'
                             .format(os.path.join(orig_log_dir, 'vecnormalize.pkl'), new_orig_log_dir))
...
def make_env_parallel(
    ...
    def _init():
        ...
        env = make_custom_env(
            seed=0, sb_version=sb_version, env_kwargs=env_kwargs, env_name=env_name, algo_name=algo_name
        )
...
def make_custom_env(
    ...
    if len(normalize_kwargs) > 0:
        env = normalize_env(env=env, sb_version=sb_version, orig_log_dir=orig_log_dir,
                            continue_learning=continue_learning,  evaluate=evaluate,
                            evaluate_during_learning=evaluate_during_learning, normalize_kwargs=normalize_kwargs)
...
def standardize_env_name(env_name: str) -> str:
    assert len(env_name.split('-')) == 2 or 'Env' in env_name, \
...
def instantiate_env_variables(algo_name, discrete_action_space, env_name, param_names=None,
                              env_values=None, model_suffix: str = None) -> EnvVariables:
    ...
    if standardize_env_name(env_name=env_name) == "cartpole":
        if env_values and evaluated_env_values:
            _env_variables = CartPoleEnvVariables(
                **evaluated_env_values, algo_name=algo_name, discrete_action_space=discrete_action_space,
                param_names=param_names, model_suffix=model_suffix
            )
        else:
            _env_variables = CartPoleEnvVariables(algo_name=algo_name, discrete_action_space=discrete_action_space,
                                                  param_names=param_names, model_suffix=model_suffix)
        ...
        if env_values and evaluated_env_values:
            ...
            _env_variables = PendulumEnvVariables(algo_name=algo_name, discrete_action_space=discrete_action_space,
                                                  param_names=param_names, model_suffix=model_suffix)
        ...
        if env_values and evaluated_env_values:
            ...
            _env_variables = MountainCarEnvVariables(algo_name=algo_name, discrete_action_space=discrete_action_space,
                                                     param_names=param_names, model_suffix=model_suffix)
        ...
        if env_values and evaluated_env_values:
            ...
            _env_variables = AcrobotEnvVariables(algo_name=algo_name, discrete_action_space=discrete_action_space,
                                                 param_names=param_names, model_suffix=model_suffix)
