...
from utilities import HOME, SUPPORTED_ALGOS, SUPPORTED_ENVS, check_param_names
...
if __name__ == "__main__":
    ...
    if args.analysis_results:
        ...
        zip_name = "analysis_{}_{}_{}.zip".format(
            standardize_env_name(args.env_name), args.algo_name, "_".join(args.param_names)
        )
    else:
        ...
        if args.folders_suffix:
            list_of_folders = (
                glob.glob(os.path.join(algo_folder, "n_iterations_{}_{}_*".format(args.folders_suffix, args.num_iterations)))
                if not args.model_suffix
                else glob.glob(
                    os.path.join(
                        algo_folder,
                        "n_iterations_{}_{}_{}_*".format(args.model_suffix, args.folders_suffix, args.num_iterations),
                    )
                )
            )
            zip_name = (
                "{}_{}_{}_{}.zip".format(
                    standardize_env_name(args.env_name), args.algo_name, args.folders_suffix, args.num_iterations
                )
                if not args.model_suffix
                else "{}_{}_{}_{}_{}.zip".format(
                    standardize_env_name(args.env_name),
                    args.algo_name,
                    args.model_suffix,
                    args.folders_suffix,
                    args.num_iterations,
            ...
            list_of_folders = (
                glob.glob(os.path.join(algo_folder, "n_iterations_{}_*".format(args.num_iterations)))
                if not args.model_suffix
                else glob.glob(
                    os.path.join(algo_folder, "n_iterations_{}_{}_*".format(args.num_iterations, args.model_suffix))
                )
            )
            zip_name = (
                "{}_{}_{}.zip".format(standardize_env_name(args.env_name), args.algo_name, args.num_iterations)
                if not args.model_suffix
                else "{}_{}_{}_{}.zip".format(
                    standardize_env_name(args.env_name), args.algo_name, args.model_suffix, args.num_iterations
                )
            )
