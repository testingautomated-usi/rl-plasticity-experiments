...
def plot_frontier_points_high_dim(
    ...
    env_values_embedded = TSNE(n_components=2, perplexity=perplexity, n_jobs=-1, n_iter=n_iterations).fit_transform(env_values)
    ...
    plot_frontier_points_2D(
        green_xs=green_xs, green_ys=green_ys, red_xs=red_xs, red_ys=red_ys, points_size=100, save_or_show=False
    )
    ...
    dot_data = tree.export_graphviz(
        clf, out_file=None, feature_names=param_names, class_names=[str(_class) for _class in clf.classes_]
    )
