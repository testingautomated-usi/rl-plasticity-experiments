...
from utilities import check_file_existence, SUPPORTED_ALGOS, SUPPORTED_ENVS, check_param_names, \
    get_result_dir_iteration_number, get_result_file_iteration_number, HOME, filter_resampling_artifacts
...
if __name__ == "__main__":
    ...
    nd_approximator = MultiDimensionalApproximator(algo_name=args.algo_name, env_name=args.env_name, model_suffix=args.model_suffix)
    ...
    filename = os.path.join(args.dir, ('analyze_volume_results_' if not args.only_tsne else 'analyze_volume_results_tsne_')
                            + 'adapt_regress_probability'
                            + ('_' + str(args.num_iterations_to_consider) if args.num_iterations_to_consider else '')
                            + '_g_' + str(args.grid_granularity_percentage_of_range)
                            + '.txt')
    ...
    list_of_config_files = glob.glob(os.path.join(env_params_dir, "*.yml")) if args.normalize_limits \
    ...
    for i, iteration_dir in enumerate(sorted_dirs):
        ...
        if os.path.isdir(iteration_dir):
            ...
            if args.num_iterations_to_consider:
                ...
                assert len(matching_files) == 1, 'There should be only one match. Found {}, {}'\
                    .format(len(matching_files), matching_files)
            ...
            adapt_volume_after_nn, adapt_volume_after_approximation, adapt_number_of_collisions, \
            adapt_frontier_pairs_collided, index_frontier_not_adapted, \
            index_frontier_not_adapted_appr, n_clusters = nd_approximator.compute_probability_volume(
            ...
            adapt_frontier_pairs_in_grid.append(int(len(read_saved_archive(archive_file=last_archive_file)) / 2) - adapt_frontier_pairs_collided)
            ...
            if args.regression_probability and not args.only_tsne:
                ...
                regress_volume_after_nn, regress_volume_after_approximation, regress_number_of_collisions, \
                regress_frontier_pairs_collided, _, _, _ = nd_approximator.compute_probability_volume(
                    buffer_file=buffer_file_resampling,
                    last_buffer_file=last_buffer_file,
                    archive_file=last_archive_file,
                    grid_granularity_percentage_of_range=args.grid_granularity_percentage_of_range,
    ...
    if not args.only_tsne:
        ...
        logger.info('Adaptation collisions percentages: {} +- {}'
                    .format(np.mean(np.array(adapt_collisions_percentages)), np.std(np.array(adapt_collisions_percentages))))
        logger.info('Adaptation volume after nn: {} +- {}'
                    .format(np.mean(np.array(adapt_volumes_after_nn)), np.std(np.array(adapt_volumes_after_nn))))
        if len(adapt_volumes_after_approximation) > 0:
            logger.info('Adaptation volume after approximation: {} +- {}'
                        .format(np.mean(np.array(adapt_volumes_after_approximation)), np.std(np.array(adapt_volumes_after_approximation))))
            logger.info('Adaptation volume diff: {} +- {}'
                        .format(np.mean(np.abs(np.array(adapt_volumes_after_approximation) - np.array(adapt_volumes_after_nn))),
                                np.std(np.abs(np.array(adapt_volumes_after_approximation) - np.array(adapt_volumes_after_nn)))))
        logger.info('Time taken for adaptation volume comp. [s]: {} +- {}'.format(np.mean(np.array(adapt_execution_times)), np.std(np.array(adapt_execution_times))))
        logger.info('Frontier pairs in adaptation grid: {} +- {}'.format(np.mean(np.array(adapt_frontier_pairs_in_grid)), np.std(np.array(adapt_frontier_pairs_in_grid))))

    if args.regression_probability:
        logger.info('********** Regression probability summary **********')
        logger.info('Regression collisions percentages: {} +- {}'
                    .format(np.mean(np.array(regress_collisions_percentages)),
                            np.std(np.array(regress_collisions_percentages))))
        logger.info('Regression volume after nn: {} +- {}'
                    .format(np.mean(np.array(regress_volumes_after_nn)), np.std(np.array(regress_volumes_after_nn))))
        if len(regress_volumes_after_approximation) > 0:
            logger.info('Regression volume after approximation: {} +- {}'
                        .format(np.mean(np.array(regress_volumes_after_approximation)),
                                np.std(np.array(regress_volumes_after_approximation))))
            logger.info('Adaptation volume diff: {} +- {}'
                        .format(np.mean(np.abs(np.array(regress_volumes_after_approximation) - np.array(regress_volumes_after_nn))),
                                np.std(np.abs(np.array(regress_volumes_after_approximation) - np.array(regress_volumes_after_nn)))))
        logger.info('Time taken for regression volume comp. [s]: {} +- {}'.format(np.mean(np.array(regress_execution_times)),
                                                                                  np.std(np.array(regress_execution_times))))
        logger.info(
            'Frontier pairs in regression grid: {} +- {}'.format(np.mean(np.array(regress_frontier_pairs_in_grid)),
                                                                 np.std(np.array(regress_frontier_pairs_in_grid))))

    if args.only_tsne:
        logger.info('********** TSNE summary **********')
        logger.info('n_clusters: {} +- {}'
                    .format(np.mean(np.array(n_clusters_all)),
                            np.std(np.array(n_clusters_all))))
