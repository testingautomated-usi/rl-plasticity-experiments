...
def is_in_archive(
    env_values: Tuple[List[float], float], archive: List[Tuple[Dict, bool]], param_names: List[str]
) -> Tuple[bool, str]:
...
def is_frontier_pair(t_env_variables: EnvVariables, f_env_variables: EnvVariables, epsilon: float, dist: float = None) -> bool:
    return is_frontier_pair_values(
        t_env_values=t_env_variables.get_values(), f_env_values=f_env_variables.get_values(), epsilon=epsilon, dist=dist
    )
...
def is_frontier_pair_values(
    t_env_values: List, f_env_values: List, epsilon: float, num_params_to_consider: int = None, dist: float = None
) -> bool:
    if not dist:
        dist = compute_dist_values(
            t_env_values=t_env_values, f_env_values=f_env_values, num_params_to_consider=num_params_to_consider
        )
...
class Archive:
    ...
    def append_best_frontier_pair(
        self, t_env_variables: EnvVariables, f_env_variables: EnvVariables, best_distance: float
    ) -> bool:
    ...
    def append(self, t_env_variables: EnvVariables, f_env_variables: EnvVariables) -> bool:
        assert is_frontier_pair(
            t_env_variables=t_env_variables, f_env_variables=f_env_variables, epsilon=self.epsilon
    ...
    def save(self, current_iteration, resampling: bool = False):
        if len(self.frontier_pairs) > 0:
            ...
            filename = (
                self.save_dir + "/frontier_" + str(current_iteration) + ".txt"
                if not resampling
                else self.save_dir + "/frontier_" + str(current_iteration) + "_resampling.txt"
            )
        ...
        if len(self.best_frontier_pairs) > 0:
            self.logger.debug("Saving archive at iteration {}".format(current_iteration))
            filename = (
                self.save_dir + "/greater_than_epsilon_pairs_" + str(current_iteration) + ".txt"
                if not resampling
                else self.save_dir + "/frontier_" + str(current_iteration) + "_resampling.txt"
            )
            with open(filename, "w+", encoding="utf-8") as f:
