...
def get_binary_search_candidate(
    ...
    while True:
        ...
        for i in range(len(t_env_variables.get_params())):
            new_value = (
                t_env_variables.get_param(index=i).get_current_value() + f_env_variables.get_param(index=i).get_current_value()
            ) / 2
        ...
        logger.info("t_env: {}, f_env: {}".format(t_env_variables.get_params_string(), f_env_variables.get_params_string()))
        ...
        for candidate_values in all_candidates:
            ...
            if not candidate_env_variables.is_equal(t_env_variables) and not candidate_env_variables.is_equal(f_env_variables):
                if not buffer_env_predicate_pairs.is_already_evaluated(candidate_env_variables=candidate_env_variables):
    ...
    assert not candidate_new_env_variables.is_equal(t_env_variables) and not candidate_new_env_variables.is_equal(
        f_env_variables
