...
from algo.archive import Archive, read_saved_archive, compute_dist, is_frontier_pair
from algo.env_exec_details import EnvExecDetails
from algo.env_predicate_pair import BufferEnvPredicatePairs, EnvPredicatePair, read_saved_buffer
...
from algo.search_utils import BufferExecutionsSkipped, ExecutionSkipped, read_saved_buffer_executions_skipped
...
class AlphaTest:
    def __init__(
        ...
        if not determine_multipliers:
            ...
            if buffer_file:
                ...
                assert executions_skipped_file, 'when buffer file is available so needs to be the executions skipped file to ' \
                                                'restore a previous execution'
                try:
                    ...
                    assert previous_num_iterations_buffer == previous_num_iterations_executions_skipped, \
                        'The two nums must coincide: {}, {}'.format(previous_num_iterations_buffer,
                                                                    previous_num_iterations_executions_skipped)
                    assert previous_num_iterations_archive == previous_num_iterations_executions_skipped, \
                        'The two nums must coincide: {}, {}'.format(previous_num_iterations_archive,
                                                                    previous_num_iterations_executions_skipped)
                ...
                self.logger.info("Restore previous execution of {} iterations. New num iterations: {}"
                                 .format(previous_num_iterations, self.num_iterations))
                ...
                algo_save_dir = os.path.abspath(HOME + "/alphatest/" + env_name + "/" + algo_name + "/" + suffix + "_" + str(attempt))
            ...
            for env_values in possible_envs:
                ...
                assert num_value_changed != 0, 'By applying multipliers we should have changed at least one value wrt the' \
                                               ' original environment. Here we changed none: {}, {}'.format(env_values, self.init_env_variables.get_values())
                ...
                if num_value_changed == 1:
                    ...
                    if len(param_changed_index) == 0:
                        param_changed_index = list(np.where(diff < 0)[0])
                    assert len(param_changed_index) == 1, 'Only one value must be > 0 or < 0. Found: {}, Diff: {}'.format(param_changed_index, diff)
            ...
            self.min_num_iterations = len(possible_envs_dict[1])  # num of possible environments considering only one param changed
            ...
            assert num_iterations >= self.min_num_iterations, 'Num iterations {} is too low. It should be at least {}'\
                .format(num_iterations, self.min_num_iterations)
    ...
    def search(self) -> Union[Archive, None]:
        ...
        if self.parallelize_search:
            ...
            for execution_result_stop in execution_results_stop:
                ...
                save_time_elapsed(save_dir=self.algo_save_dir,
            ...
            while not queue_result.empty():
                t_env_variables, f_env_variables, env_predicate_pairs,\
                    max_binary_search_iterations, binary_search_counter, \
                    current_iteration, execution_times, regression_times = queue_result.get_nowait()
                ...
                if binary_search_counter == max_binary_search_iterations:
                    ...
                    self.archive.append(
                        t_env_variables=t_env_variables[0],
                        f_env_variables=f_env_variables[1]
                    )
                    self.buffer_env_predicate_pairs.save(current_iteration=current_iteration)
                    self.archive.save(current_iteration=current_iteration)
                    self._move_output_directories(current_iteration=current_iteration)
                ...
                save_time_elapsed(save_dir=self.algo_save_dir, current_iteration=current_iteration,
                                  execution_times=regression_times, regression=True)
        else:
            for current_iteration in range_fn:
                ...
                while (
                    ...
                    new_env_variables = self._binary_search(
                        t_env_variables=t_env_variables,
                        f_env_variables=f_env_variables,
                    )
                    ...
                    if self.binary_search_guidance and executed_env_dominate_false:
                        assert executed_env_dominate_true is None, 'it can\'t be that env {} dominates False env {} and is dominated by True env {}'.format(
                            new_env_variables.get_params_string(), executed_env_dominate_false.get_env_variables().get_params_string(),
                        ...
                        self.logger.debug('binary_search_guidance: env {} not executed because dominated by True env {}'.format(
                            new_env_variables.get_params_string(), executed_env_dominate_true.get_env_variables().get_params_string())
                        )
                        env_predicate_pair = EnvPredicatePair(
                            env_variables=new_env_variables,
                    ...
                    t_pass_probability = t_env_predicate_pair.get_pass_probability() \
                        if t_env_predicate_pair.get_pass_probability() else t_env_predicate_pair.compute_pass_probability()
                    f_pass_probability = f_env_predicate_pair.get_pass_probability() \
                        if f_env_predicate_pair.get_pass_probability() else f_env_predicate_pair.compute_pass_probability()
                ...
                if binary_search_counter == max_binary_search_iterations:
                    self.logger.info("Binary search did not converge in " + str(max_binary_search_iterations) + " steps")
                    self.logger.info("Reporting the closest pair found, t_env: {}, f_env: {}, dist: {}, epsilon: {}".format(
                        best_t_f_env_variables[0].get_params_string(),
                        best_t_f_env_variables[1].get_params_string(), dist, self.binary_search_epsilon
                    ))
    ...
    def _move_output_directories(self, current_iteration, resampling: bool = False) -> None:
        ...
        dir_to_create = self.algo_save_dir + "/iteration_" + str(current_iteration) if not resampling \
    ...
    def _exp_search(
        ...
        for num_param_changed in self.possible_envs_dict.keys():
            candidates = list(filter(lambda env_exec_details: not env_exec_details.is_executed(),
                                     self.possible_envs_dict[num_param_changed]))
        ...
        self.logger.debug('Setting candidate {} as executed {}'.format(
        ...
        if self.exp_search_guidance and executed_env_dominate_false:
            ...
            self.logger.debug('exp_search_guidance: env {} not executed because it dominates False env {}'.format(
                candidate_new_env_variables.get_params_string(), executed_env_dominate_false.get_env_variables().get_params_string())
            )
            ...
            self.logger.debug('exp_search_guidance: env {} not executed because dominated by True env {}'.format(
                candidate_new_env_variables.get_params_string(),
                executed_env_dominate_true.get_env_variables().get_params_string())
            )
        ...
        while True:
            ...
            for i in range(len(t_env_variables.get_params())):
                ...
                if i not in candidates_dict:
                    candidates_dict[i] = []
                if t_env_variables.get_param(index=i).get_current_value() != f_env_variables.get_param(index=i).get_current_value():
            ...
            self.logger.debug('t_env: {}, f_env: {}'.format(
            ...
            if len(all_candidates_env_variables_filtered) > 0:
                ...
                for candidate_env_variables in all_candidates_env_variables:
                    if not candidate_env_variables.is_equal(t_f_env_variables[0]):
                        all_candidates_env_variables_filtered.append(candidate_env_variables)
                assert len(all_candidates_env_variables_filtered) > 0, 'there must be at least one candidate env for binary search'
    ...
    def resample(self):
        ...
        for i in range(len(self.init_env_variables.get_params())):
            ...
            assert param.get_direction() == 'positive' or param.get_direction() == 'negative', \
                'Direction either positive or negative. Found {} for param {}'.format(
                    param.get_direction(), param.get_name())
            limit = None
            ...
            for env_predicate_pair in self.buffer_env_predicate_pairs.get_buffer():
                ...
                if env_predicate_pair.is_predicate() and other_param.get_current_value() == limit:
                    self.logger.info('Env to execute for param {} with limit {}: {}'.format(
        ...
        for index_param, list_of_env_variables_to_execute in env_variables_to_execute_per_param:
            ...
            for env_variables_to_execute in list_of_env_variables_to_execute:
                ...
                self.logger.info('Current param value for param {}: {}'.format(
                ...
                while predicate:
                    ...
                    env_variables_to_execute_modified.set_param(
                        index=index_param, new_value=env_variables_to_execute_modified.get_param(index=index_param).get_current_value() * multiplier,
                        do_not_check_for_limits=True)
                    self.logger.info('New param value for param {}: {}'.format(
                        env_variables_to_execute_modified.get_param(index=index_param).get_name(),
                        env_variables_to_execute_modified.get_param(index=index_param).get_current_value()))
                    env_predicate_pair, execution_time, regression_time = self.runner.execute_train(
                        current_iteration=current_iteration,
                    ...
                    if env_predicate_pair.compute_pass_probability() == 0.0:
                        self.logger.info('Stop resampling. Found value {} of param {} that invalidates the env'.format(
                            env_variables_to_execute_modified.get_param(index=index_param).get_current_value(),
                            env_variables_to_execute_modified.get_param(index=index_param).get_name()))
                        predicate = env_predicate_pair.is_predicate()
    ...
    def determine_multipliers(self, num_of_runs: int, halve_or_double: str = "double") -> Tuple[List[float], List[float]]:
        assert halve_or_double == 'double' or halve_or_double == 'halve', "halve_or_double == double or halve, {}"\
            .format(halve_or_double)
            ...
            for index in range(len(current_env_variables.get_params())):
                ...
                if not env_predicate_pair.is_predicate():
                    ...
                    if percentage_drop >= 100.0 or mean_reward <= current_env_variables.get_lower_limit_reward():
                        ...
                        while True:
                            self.logger.debug('Try changing the param with baby steps. Mul {}, new value: {}'
                                              .format(mul, param.get_current_value() * mul))
